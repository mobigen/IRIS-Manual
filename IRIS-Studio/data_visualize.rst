...
- 기본 지도 레이어로 "layer map" 레이어가 있으며, 레이어 관리를 통해서 layer map 위에 표시할 데이터가 있는 레이터를 추가합니다.
...
| 데이터 탭의 ``레이어 선택`` 에서 "layer map" 을 선택했을 때 메뉴 및 버튼에 관한 설명입니다.
...
| 기본 레이어인 "layer map" 만 있다면 레이어 선택 옆의 ``관리`` 버튼을 클릭하면 레이어 관리 팝업이 열립니다.
...
| 트리거 이벤트 발생 시 전달받은 위/경도 좌표로 지도의 기본 위치가 이동하며, 설정한 줌레벨로 변경되어 ``layer map``  레이어가 표시됩니다.
...
| 지도 객체는 레이어마다 오브젝트ID 가 부여되고, 기본 레이어인 "layer map"  도 ``map-번호`` 형식으로 assign 됩니다.
...
- 지도 투명도 
    - layer map 레이어의 바탕 지도에 대해 투명도를 적용할 수 있습니다. 0 으로 셋팅하면 layer map 이 보이지 않는 효과가 있습니다.
...
- 히트맵
    - 적용되는 시각화 유형 : 히트맵
    - 지도에 표시될 히트맵의 크기(영향을 미치는 범위)를 ``포인트별 반지름`` 을 고정길이(픽셀단위), 거리기준 으로 할 것인지 설정합니다.
...
| 항목탭은 기본 레이어(layer map(지도)) 에서는 보이지 않고, 추가하는 데이터 레이어에서 활성화되어 보입니다. 
