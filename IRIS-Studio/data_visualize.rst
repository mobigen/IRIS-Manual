...
| 레이어 선택 메뉴에서 기본 layer map 위에 추가하는 레이어에 대한 설정입니다.
...
.. image:: ./studio/images/map/map_0831_06_2.png
    :scale: 60%
    :alt: 데이터 설정
...
| 데이터 탭에서 데이터를 설정한 후 **실행** 을 클릭하여 데이터를 가져온 후에 Map의 시각화 옵션을 설정할 수 있습니다.
...
| 좌표를 마커로 표시하는 시각화 방법은 포인트와 깃발 중에서 선택 할 수 있습니다.
| 
| (2020.08월 추가) 데이터 탭에서 마커에 해당하는 좌표마다 순번(sequence)이 있는 데이터인 경우에는 "경로 순서" 데이터 컬럼으로 지정하여 마커 지점과 마커 지점사이를
| 화살표 선으로 경로를 표시할 수 있습니다.
...
| 항목탭은 지도에서 시각화유형(마커,도형,타일,mesh,라벨)으로 표현할 때, 특정 조건일 때의 항목을 설정해서 해당 부분을 강조색상으로 표현할 때 사용됩니다.
| 
| 선택한 레이어의 컬럼(항목 선택)의 값이 트리거 이벤트로 받은 변수(항목 설정 변수)값과 일치하는 경우에는
| 시각화 유형의 시각화 옵션의 색상에서 **강조 색상** 으로 해당 값이 변경됩니다.

| 항목탭은 기본 레이어(layer map(지도)) 에서는 보이지 않고, 추가 레이어에서 활성화되어 보입니다. 
